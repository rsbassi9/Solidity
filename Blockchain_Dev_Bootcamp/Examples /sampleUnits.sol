...
// A better way to write those units is to internally denominate in ether: (other units: wei, gwei, ether)
...
// Time: for example, if you have a crowd auction that starts on x days and runs for 7 days:
    ...
    // in Solidity, timestamps are in unix (seconds) by default, but theres a better way to write them: (other units: seconds, minutes, hours, days, weeks)
